...
class Route53Backend(connections: List[Route53BackendConnection]) extends Backend {

  private val connMap: Map[String, Route53BackendConnection] = connections.map(c => c.id -> c).toMap
