...
object ZoneChangeGenerator {
  ...
  def forAdd(
    ...
    ZoneChange(
      zone
        .copy(id = UUID.randomUUID().toString, created = Instant.now, status = ZoneStatus.Syncing),
  ...
  def forUpdate(
    ...
    ZoneChange(
      newZone.copy(updated = Some(Instant.now), connection = fixConn(oldZone, newZone, crypto)),
      authPrincipal.userId,
  ...
  def forSync(zone: Zone, authPrincipal: AuthPrincipal): ZoneChange =
    ZoneChange(
      zone.copy(updated = Some(Instant.now), status = ZoneStatus.Syncing),
      authPrincipal.userId,
      ZoneChangeType.Sync,
      ZoneChangeStatus.Pending
    )
  ...
  def forDelete(zone: Zone, authPrincipal: AuthPrincipal): ZoneChange =
    ZoneChange(
      zone.copy(updated = Some(Instant.now), status = ZoneStatus.Deleted),
      authPrincipal.userId,
