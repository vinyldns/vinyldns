...
class ZoneService(
  ...
  def connectToZone(
    ...
    for {
      ...
      _ <- connectionValidator.hasExistingBackendId(createZoneInput.backendId).toResult
  ...
  def updateZone(updateZoneInput: UpdateZoneInput, auth: AuthPrincipal): Result[ZoneCommandResult] =
    for {
      ...
      _ <- connectionValidator.hasExistingBackendId(updateZoneInput.backendId).toResult
