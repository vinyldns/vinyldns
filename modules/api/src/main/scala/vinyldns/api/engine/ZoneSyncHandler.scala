...
object ZoneSyncHandler extends DnsConversions with Monitored {
  ...
  def apply(
      ...
      implicit executionContext: ExecutionContext): ZoneChange => IO[ZoneChange] =
    zoneChange =>
      IO.fromFuture(IO {
          monitor("zone.sync") {
            time(s"zone.sync(${zoneChange.zoneId})") {
              ...
              val recordSetChanges =
                for {
                  dnsZoneView <- dnsView
                  vinyldnsZoneView <- vinyldnsView
                } yield vinyldnsZoneView.diff(dnsZoneView)

              recordSetChanges.flatMap {
                allChanges =>
                  ...
                  if (changes.isEmpty) {
                    ...
                    Future.successful(zoneChange.copy(
