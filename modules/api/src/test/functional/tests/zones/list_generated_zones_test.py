...
def list_generated_zone_context(shared_zone_test_context):
    return shared_zone_test_context.list_generated_zones


def test_list_generated_zones_success(list_generated_zone_context, shared_zone_test_context):
    """
    Test that we can retrieve a list of the user's zones
    """
    result = shared_zone_test_context.list_generated_zones_client.list_generated_zones(name_filter=f"*{shared_zone_test_context.partition_id}", status=200)
...
def test_list_generated_zones_by_admin_group_name(list_generated_zone_context, shared_zone_test_context):
    """
    Test that we can retrieve list of zones by searching with admin group name
    """
    result = shared_zone_test_context.list_generated_zones_client.list_generated_zones(
        name_filter=f"list-generated-zones-group{shared_zone_test_context.partition_id}",
        search_by_admin_group=True,
        status=200)
    retrieved = result["zones"]
    ...
    assert_that(retrieved, has_item(has_entry("zoneName", list_generated_zone_context.non_search_generate_zone1["zoneName"])))
    assert_that(retrieved, has_item(has_entry("zoneName", list_generated_zone_context.non_search_generate_zone2["zoneName"])))
    assert_that(retrieved, has_item(has_entry("groupName", list_generated_zone_context.list_generate_zones_group["name"])))

    assert_that(result["nameFilter"], is_(f"list-generated-zones-group{shared_zone_test_context.partition_id}"))


def test_list_generated_zones_by_admin_group_name_with_wildcard(list_generated_zone_context, shared_zone_test_context):
    """
...
def test_list_generated_zones_max_items_100(shared_zone_test_context):
    """
...
def test_list_generated_zones_invalid_max_items_fails(shared_zone_test_context):
    ...
    """
    errors = shared_zone_test_context.list_generated_zones_client.list_generated_zones(max_items=700, status=400)
    assert_that(errors, contains_string("maxItems was 700, maxItems must be between 0 and 100"))


def test_list_generated_zones_no_authorization(shared_zone_test_context):
    """
    Test that we cannot retrieve a list of zones without authorization
    """
    shared_zone_test_context.list_generated_zones_client.list_generated_zones(sign_request=False, status=401)


def test_list_generated_zones_no_search_first_page(list_generated_zone_context, shared_zone_test_context):
    """
    Test that the first page of listing zones returns correctly when no name filter is provided
    """
    result = shared_zone_test_context.list_generated_zones_client.list_generated_zones(name_filter=f"*{shared_zone_test_context.partition_id}", max_items=3)
    zones = result["zones"]
...
def test_list_generated_zones_no_search_second_page(list_generated_zone_context, shared_zone_test_context):
    """
    Test that the second page of listing zones returns correctly when no name filter is provided
    """
    result = shared_zone_test_context.list_generated_zones_client.list_generated_zones(name_filter=f"*{shared_zone_test_context.partition_id}",
                                                                   start_from=list_generated_zone_context.search_generate_zone2["zoneName"],
                                                                   max_items=2,
                                                                   status=200)
    zones = result["zones"]
    ...
    assert_that(zones[1]["zoneName"], is_(list_generated_zone_context.non_search_generate_zone1["zoneName"]))

    assert_that(result["nextId"], is_(list_generated_zone_context.non_search_generate_zone1["zoneName"]))
...
def test_list_generated_zones_no_search_last_page(list_generated_zone_context, shared_zone_test_context):
    """
    Test that the last page of listing zones returns correctly when no name filter is provided
    """
    result = shared_zone_test_context.list_generated_zones_client.list_generated_zones(name_filter=f"*{shared_zone_test_context.partition_id}",
                                                                   start_from=list_generated_zone_context.search_generate_zone3["zoneName"],
                                                                   max_items=4,
                                                                   status=200)
    ...
    assert_that(result["startFrom"], is_(list_generated_zone_context.search_generate_zone3["zoneName"]))
    assert_that(result["nameFilter"], is_(f"*{shared_zone_test_context.partition_id}"))


def test_list_generated_zones_with_search_first_page(list_generated_zone_context, shared_zone_test_context):
    """
    Test that the first page of listing zones returns correctly when a name filter is provided
    """
    result = shared_zone_test_context.list_generated_zones_client.list_generated_zones(name_filter=f"*searched*{shared_zone_test_context.partition_id}", max_items=2, status=200)
    zones = result["zones"]
...
def test_list_generated_zones_with_no_results(shared_zone_test_context):
    """
    Test that the response is formed correctly when no results are found
    """
    result = shared_zone_test_context.list_generated_zones_client.list_generated_zones(name_filter="this-wont-be-found", max_items=2, status=200)
    zones = result["zones"]
...
def test_list_generated_zones_with_search_last_page(list_generated_zone_context, shared_zone_test_context):
    """
    Test that the second page of listing zones returns correctly when a name filter is provided
    """
    result = shared_zone_test_context.list_generated_zones_client.list_generated_zones(name_filter=f"*test-searched-3{shared_zone_test_context.partition_id}",
                                                                   start_from=list_generated_zone_context.search_generate_zone2["zoneName"],
                                                                   max_items=2,
                                                                   status=200)
    zones = result["zones"]
