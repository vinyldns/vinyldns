  ...
  "RecordSetValidations" should {
    ...
    "isDotted" should {
      "return a failure for any record with dotted hosts if it is already present" in {
        val test = aaaa.copy(name = "this.is.a.failure.")
        leftValue(isDotted(test, okZone, None, false, true, false)) shouldBe an[InvalidRequest]
      }

      "return a failure for any record that is a dotted host if user or record type is not allowed" in {
        val test = aaaa.copy(name = "this.is.a.failure." + okZone.name)
        leftValue(isDotted(test, okZone, None, true, false, false)) shouldBe an[InvalidRequest]
      }

      "return success for a dotted record if it does not already have a record or zone with same name and user is allowed" in {
        val test = aaaa.copy(name = "this.passes")
        isDotted(test, okZone, None, true, true, false) should be(right)
      }
      ...
      "return success for a new record that has the same name as the existing record" in {
        ...
        isDotted(newRecord, okZone, Some(existingRecord), true, true, false) should be(right)
      }
    }
    ...
    "typeSpecificValidations" should {
      "Run dotted hosts checks" should {
        ...
        "return a failure for any new record with dotted hosts in forward zones" in {
          leftValue(
            typeSpecificValidations(dottedARecord, List(), okZone, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, false, 0 , false)
          ) shouldBe an[InvalidRequest]
        }

        "return a failure for any new record with dotted hosts in forward zones (CNAME)" in {
          leftValue(
            typeSpecificValidations(dottedARecord.copy(typ = CNAME), List(), okZone, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, false, 0, false)
          ) shouldBe an[InvalidRequest]
        }

        "return a failure for any new record with dotted hosts in forward zones (NS)" in {
          leftValue(
            typeSpecificValidations(dottedARecord.copy(typ = NS), List(), okZone, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, false, 0, false )
          ) shouldBe an[InvalidRequest]
        }
        ...
        "return a success for any new record with dotted hosts in forward zones if it satisfies dotted hosts configs" in {
          ...
          val record = typeSpecificValidations(dottedARecord.copy(typ = CNAME, zoneId = dottedZone.id), List(), dottedZone, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, true, 5, false)
        ...
        "return a failure for any new record with dotted hosts if no.of.dots allowed is 0" in {
          // Zone, User, Record Type and Number of dots are all satisfied
          leftValue(
            typeSpecificValidations(dottedARecord.copy(typ = CNAME, zoneId = dottedZone.id), List(), dottedZone, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, true, 0, false)
          ) shouldBe an[InvalidRequest]
        }
        ...
        "return a failure for any new record with dotted hosts in forward zones (A record) if it doesn't satisfy dotted hosts configs" in {
          ...
          leftValue(
            typeSpecificValidations(dottedARecord.copy(zoneId = dottedZone.id), List(), dottedZone, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, false, 5, false)
          ) shouldBe an[InvalidRequest]
        }
        ...
        "return a failure for any new record with dotted hosts in forward zones (NS record) if it doesn't satisfy dotted hosts configs" in {
          ...
          leftValue(
            typeSpecificValidations(dottedARecord.copy(typ = NS, zoneId = dottedZone.id), List(), dottedZone, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, false, 5, false)
          ) shouldBe an[InvalidRequest]
        }
      ...
      "Skip dotted checks on SRV" should {
        "return success for an SRV record following convention with FQDN" in {
          ...
          typeSpecificValidations(test, List(), zone, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, false, 0, false) should be(right)
        }
      ...
      "Skip dotted checks on SOA in reverse zones" should {
        "return success for an SOA record with dots in a reverse zone" in {
          ...
          typeSpecificValidations(test, List(), zoneIp4, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, false , 0, false) should be(right)
        }
      }
    }
    ...
    "NSValidations" should {
      ...
      "return ok if the record is an NS record but not origin" in {
        ...
        nsValidations(valid, okZone, None, List(new Regex(".*")), true, dottedHostsConfigZonesAllowed.toSet, false, 0, false) should be(right)
      }

      "return an InvalidRequest if an NS record is '@'" in {
        val error = leftValue(nsValidations(invalidNsApexRs, okZone, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, false , 0, false))
        error shouldBe an[InvalidRequest]
      }

      "return an InvalidRequest if an NS record is the same as the zone" in {
        val invalid = invalidNsApexRs.copy(name = okZone.name)
        val error = leftValue(nsValidations(invalid, okZone, None, Nil, true, dottedHostsConfigZonesAllowed.toSet, false, 0, false))
        error shouldBe an[InvalidRequest]
      }

      "return an InvalidRequest if the NS record being updated is '@'" in {
        val valid = invalidNsApexRs.copy(name = "this-is-not-origin-mate")
        val error = leftValue(nsValidations(valid, okZone, Some(invalidNsApexRs), Nil, true, dottedHostsConfigZonesAllowed.toSet, false , 0, false))
        error shouldBe an[InvalidRequest]
      }
      ...
      "return an InvalidRequest if an NS record data is not in the approved server list" in {
        ...
        val error = leftValue(nsValidations(ns, okZone, None, List(new Regex("not.*")), true, dottedHostsConfigZonesAllowed.toSet, false , 0, false))
        error shouldBe an[InvalidRequest]
      }
    }
    ...
    "DSValidations" should {
      ...
      "return ok if the record is non-origin DS with matching NS" in {
        dsValidations(ds, List(matchingNs), okZone, true, dottedHostsConfigZonesAllowed.toSet, false , 0, false) should be(right)
      }
      "return an InvalidRequest if a DS record is '@'" in {
        val apex = ds.copy(name = "@")
        val error = leftValue(dsValidations(apex, List(matchingNs), okZone, true, dottedHostsConfigZonesAllowed.toSet, false, 0, false))
        error shouldBe an[InvalidRequest]
      }
      ...
      "return an InvalidRequest if there is no NS matching the record" in {
        val error = leftValue(dsValidations(ds, List(), okZone, true, dottedHostsConfigZonesAllowed.toSet, false, 0, false))
        error shouldBe an[InvalidRequest]
      }
      "return an InvalidRequest if the DS is dotted" in {
        val error =
          leftValue(dsValidations(ds.copy(name = "test.dotted"), List(matchingNs), okZone, true, dottedHostsConfigZonesAllowed.toSet, false , 0, false))
        error shouldBe an[InvalidRequest]
      }
      "return ok if the DS is dotted and zone, user, record type is allowed in dotted hosts config" in {
        val record =
          dsValidations(ds.copy(name = "dotted.trial", zoneId = dottedZone.id), List(matchingNs), dottedZone, true, dottedHostsConfigZonesAllowed.toSet, true, 5 , false)
        record should be(right)
      }
      "return an InvalidRequest if the DS is dotted and zone, user, record type is allowed in dotted hosts config but has a conflict with existing record or zone" in {
        val error =
          leftValue(dsValidations(ds.copy(name = "dotted.trial", zoneId = dottedZone.id), List(matchingNs), dottedZone, false, dottedHostsConfigZonesAllowed.toSet, true , 0, false))
        error shouldBe an[InvalidRequest]
      }
    }
    ...
    "CnameValidations" should {
      ...
      "return a RecordSetAlreadyExistsError if a record with the same name exists and creating a cname" in {
        val error = leftValue(cnameValidations(cname, List(aaaa), okZone, None, true, dottedHostsConfigZonesAllowed.toSet, false , 0, false))
        error shouldBe a[RecordSetAlreadyExists]
      }
      "return ok if name is not '@'" in {
        cnameValidations(cname, List(), okZone, None, true, dottedHostsConfigZonesAllowed.toSet, false, 0, false) should be(right)
      }
      "return an InvalidRequest if a cname record set name is '@'" in {
        val error = leftValue(cnameValidations(invalidCnameApexRs, List(), okZone, None,  true, dottedHostsConfigZonesAllowed.toSet, false , 0, false))
        error shouldBe an[InvalidRequest]
      }
      "return an InvalidRequest if a cname record set name is same as zone" in {
        ...
        val error = leftValue(cnameValidations(invalid, List(), okZone, None, true, dottedHostsConfigZonesAllowed.toSet, false, 0, false))
        error shouldBe an[InvalidRequest]
      }
      ...
      "return an InvalidRequest if a cname record set name is dotted" in {
        val error = leftValue(cnameValidations(cname.copy(name = "dot.ted"), List(), okZone, None, true, dottedHostsConfigZonesAllowed.toSet, false , 0, false))
        error shouldBe an[InvalidRequest]
      }
      "return ok if new recordset name does not contain dot" in {
        cnameValidations(cname, List(), okZone, Some(cname.copy(name = "not-dotted")), true, dottedHostsConfigZonesAllowed.toSet, false , 0, false) should be(
          right
        )
      }
      "return ok if dotted host name doesn't change" in {
        val newRecord = cname.copy(name = "dot.ted", ttl = 500)
        cnameValidations(newRecord, List(), okZone, Some(newRecord.copy(ttl = 300)), true, dottedHostsConfigZonesAllowed.toSet, false , 0, false) should be(
          right
        )
      }
      "return an InvalidRequest if a cname record set name is updated to '@'" in {
        val error = leftValue(cnameValidations(cname.copy(name = "@"), List(), okZone, Some(cname), true, dottedHostsConfigZonesAllowed.toSet, false , 0, false))
        error shouldBe an[InvalidRequest]
      }
      "return an InvalidRequest if updated cname record set name is same as zone" in {
        val error =
          leftValue(cnameValidations(cname.copy(name = okZone.name), List(), okZone, Some(cname), true, dottedHostsConfigZonesAllowed.toSet, false , 0, false))
        error shouldBe an[InvalidRequest]
      }
      "return an RecordSetValidation error if recordset data contain more than one sequential '.'" in {
        val error = leftValue(cnameValidations(cname.copy(records = List(CNAMEData(Fqdn("record..zone")))), List(), okZone, None, true, dottedHostsConfigZonesAllowed.toSet, false , 0, false))
        error shouldBe an[RecordSetValidation]
      }
      "return ok if recordset data does not contain sequential '.'" in {
        cnameValidations(cname.copy(records = List(CNAMEData(Fqdn("record.zone")))), List(), okZone, None, true, dottedHostsConfigZonesAllowed.toSet, false , 0, false) should be(
          right
        )
      }
      "return ok if the CNAME is dotted and zone, user, record type is allowed in dotted hosts config" in {
        val record =
          cnameValidations(cname.copy(name = "dot.ted", zoneId = dottedZone.id), List(), dottedZone, None, true, dottedHostsConfigZonesAllowed.toSet, true, 5, false)
        record should be(right)
      }
      "return an InvalidRequest if the CNAME is dotted and zone, user, record type is allowed in dotted hosts config but has a conflict with existing record or zone" in {
        val error =
          leftValue(cnameValidations(cname.copy(name = "dot.ted", zoneId = dottedZone.id), List(), dottedZone, None, false, dottedHostsConfigZonesAllowed.toSet, true , 0, false))
        error shouldBe an[InvalidRequest]
      }
    }
