...
def test_create_batch_change_for_shared_zone_with_owner_group_id_succeeds(shared_zone_test_context):
    """
    Test successfully creating a batch change with owner group ID in shared zone
    """
...
def test_create_batch_change_for_shared_zone_without_owner_group_id_fails(shared_zone_test_context):
    """
    Test creating batch change for shared zone without owner group ID fails
    """
    shared_client = shared_zone_test_context.shared_zone_vinyldns_client
    shared_zone = shared_zone_test_context.shared_zone
    ...
    try:
        ...
        response = shared_client.create_batch_change(batch_change_input, status=400)

        assert_failed_change_in_error_response(response[0], input_name="no-owner-group-id.shared.", record_data="4.3.2.1",
                                               error_messages=['Zone "shared." is a shared zone, so owner group ID must be specified for record "no-owner-group-id".'])
        assert_failed_change_in_error_response(response[1], input_name="update.shared.", record_data="1.2.3.4",
                                               error_messages=['Zone "shared." is a shared zone, so owner group ID must be specified for record "update".'])
        assert_successful_change_in_error_response(response[2], change_type="DeleteRecordSet", input_name="update.shared.", record_data=None)
    finally:
        clear_recordset_list(to_delete, shared_client)
