...
def test_create_recordset_with_record_ownergroup(shared_zone_test_context):
    ...
    try:
        ...
        new_rs = {
            ...
            'name': 'test_create_recordset_with_ownergroup',
            'type': 'A',
            'ttl': 100,
            'records': [
                {
        ...
        print "\r\nCreating recordset in zone " + str(shared_zone_test_context.shared_zone) + "\r\n"
        result = client.create_recordset(new_rs, status=202)
        print str(result)
        ...
        assert_that(result_rs['ownerGroupId'], is_(created_group['id']))
        records = [x['address'] for x in result_rs['records']]
        assert_that(records, has_length(2))
        assert_that('10.1.1.1', is_in(records))
        assert_that('10.2.2.2', is_in(records))
        ...
        print "\r\n\r\n!!!verifying recordset in dns backend"
        answers = dns_resolve(shared_zone_test_context.shared_zone, result_rs['name'], result_rs['type'])
        rdata_strings = rdata(answers)
