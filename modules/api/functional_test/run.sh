...
echo "./run-tests.py live_tests -n${PAR_CPU} -v --url=${VINYLDNS_URL} --dns-ip=${DNS_IP} ${TEST_PATTERN} --teardown=true"
./run.py live_tests -n${PAR_CPU} -v --url=${VINYLDNS_URL} --dns-ip=${DNS_IP} ${TEST_PATTERN} --teardown=true
result=$?
