...
object Messages {
  ...
  val GroupEmailExistsErrorMsg: String =
  ...
  val ACLRuleErrorMsg: String =
    "%s has an ACL rule for a zone including %s. Cannot delete. Please transfer the ownership to another group before deleting.".orConfig

  val RecordSetOwnerErrorMsg: String =
    "%s is the owner for a record set including %s. Cannot delete. Please transfer the ownership to another group before deleting.".orConfig

  val GroupAlreadyExistsErrorMsg: String =
    "Group with name %s already exists. Please try a different name or contact %s to be added to the group.".orConfig

  val ZoneAdminErrorMsg: String =
    "%s is the admin of a zone. Cannot delete. Please transfer the ownership to another group before deleting.".orConfig

  /* DomainValidationErrors.scala */

  val ChangeLimitExceededErrorMsg: String =
    "Cannot request more than %d changes in a single batch change request".orConfig

  val BatchChangeIsEmptyErrorMsg: String =
    "Batch change contained no changes. Batch change must have at least one change, up to a maximum of %d changes.".orConfig

  val NotMemberOfGroupErrorMsg: String =
    "User \"%s\" must be a member of group \"%s\" to apply this group to batch changes.".orConfig

  val InvalidDomainNameErrorMsg: String =
    "Invalid domain name: \"%s\", valid domain names must be letters, numbers, underscores, and hyphens, joined by dots, and terminated with a dot.".orConfig

  val InvalidLengthErrorMsg: String =
    "Invalid length: \"%s\", length needs to be between %d and %d characters.".orConfig

  val InvalidEmailErrorMsg: String = "Invalid email address: \"%s\".".orConfig

  val InvalidRecordTypeErrorMsg: String =
    "Invalid record type: \"%s\", valid record types include %s.".orConfig

  val InvalidPortNumberErrorMsg: String =
    "Invalid port number: \"%s\", port must be a number between %d and %d.".orConfig
  ...
  val InvalidTTLErrorMsg: String =
    "Invalid TTL: \"%s\", must be a number between %d and %d.".orConfig

  val InvalidMxPreferenceErrorMsg: String =
    "Invalid MX Preference: \"%s\", must be a number between %d and %d.".orConfig

  val InvalidBatchRecordTypeErrorMsg: String =
    "Invalid Batch Record Type: \"%s\", valid record types for batch changes include %s.".orConfig

  val ZoneDiscoveryErrorMsg: String =
    "Zone Discovery Failed: zone for \"%s\" does not exist in VinylDNS. If zone exists, then it must be connected to in VinylDNS.".orConfig

  val RecordAlreadyExistsErrorMsg: String =
    "Record \"%s\" Already Exists: cannot add an existing record; to update it, issue a DeleteRecordSet then an Add.".orConfig

  val RecordDoesNotExistErrorMsg: String =
    "Record \"%s\" Does Not Exist: cannot delete a record that does not exist.".orConfig

  val CnameIsNotUniqueErrorMsg: String =
    "CNAME Conflict: CNAME record names must be unique. Existing record with name \"%s\" and type \"%s\" conflicts with this record.".orConfig

  val RecordNameNotUniqueInBatchErrorMsg: String =
    "Record Name \"%s\" Not Unique In Batch Change: cannot have multiple \"%s\" records with the same name.".orConfig

  val RecordInReverseZoneErrorMsg: String =
    "Invalid Record Type In Reverse Zone: record with name \"%s\" and type \"%s\" is not allowed in a reverse zone.".orConfig

  val HighValueDomainErrorMsg: String =
    "Record name \"%s\" is configured as a High Value Domain, so it cannot be modified.".orConfig

  val MissingOwnerGroupIdErrorMsg: String =
    "Zone \"%s\" is a shared zone, so owner group ID must be specified for record \"%s\".".orConfig

  val CnameAtZoneApexErrorMsg: String = "CNAME cannot be the same name as zone \"%s\".".orConfig

  val RecordManualReviewErrorMsg: String =
    "Record set with name \"%s\" requires manual review.".orConfig

  val UnsupportedOperationErrorMsg: String =
    "%s is not yet implemented/supported in VinylDNS.".orConfig

  val DeleteRecordDataDoesNotExistErrorMsg: String =
    "Record data %s does not exist for \"%s\".".orConfig

  val NotAuthorizedErrorMsg: String =
    "User \"%s\" is not authorized. Contact %s owner group: %s at %s to make DNS changes.".orConfig

  /* RecordSetValidations.scala */

  val RecordNameFilterErrorMsg: String =
    "Search must contain at least two letters or numbers to perform a RecordSet Search.".orConfig
  ...
  val RecordOwnerGroupNotFoundErrorMsg: String =
    "Record owner group with id \"%s\" not found".orConfig

  val UserNotInOwnerGroupErrorMsg: String = "User not in record owner group with id \"%s\"".orConfig

  val NSApexErrorMsg: String =
    "Record with name %s is an NS record at apex and cannot be added".orConfig

  val NSApexEditErrorMsg: String =
    "Record with name %s is an NS record at apex and cannot be edited".orConfig

  val DSTTLErrorMsg: String = "DS record [%s] must have TTL matching its linked NS (%d)".orConfig

  val DSInvalidErrorMsg: String =
    "DS record [%s] is invalid because there is no NS record with that name in the zone [%s]".orConfig

  val DSApexErrorMsg: String =
    "Record with name [%s] is an DS record at apex and cannot be added".orConfig

  val CnameDuplicateErrorMsg: String =
    "RecordSet with name %s already exists in zone %s, CNAME record cannot use duplicate name".orConfig

  val InvalidCnameErrorMsg: String =
    "CNAME RecordSet cannot have name '@' because it points to zone origin".orConfig

  val SOADeleteErrorMsg: String = "SOA records cannot be deleted".orConfig

  val DottedHostErrorMsg: String =
    "Record with name %s and type %s is a dotted host which is not allowed in zone %s".orConfig

  val RecordSetAlreadyExistsErrorMsg: String =
    "RecordSet with name %s and type %s already exists in zone %s".orConfig

  val RecordSetCnameExistsErrorMsg: String =
    "RecordSet with name %s and type CNAME already exists in zone %s".orConfig

  val PendingUpdateErrorMsg: String =
    "RecordSet with id %s, name %s and type %s currently has a pending change".orConfig
  ...
  val BatchConversionErrorMsg: String =
    "Batch conversion for processing failed to convert change with name \"%s\" and type \"%s\"".orConfig

  val BatchChangeNotPendingReviewErrorMsg: String =
    "Batch change %s is not pending review, so it cannot be rejected.".orConfig

  val BatchRequesterNotFoundErrorMsg: String =
    "The requesting user with id %s and name %s cannot be found in VinylDNS".orConfig

  val ScheduledChangesDisabledErrorMsg: String =
    "Cannot create a scheduled change, as it is currently disabled on this VinylDNS instance.".orConfig

  val ScheduledTimeMustBeInFutureErrorMsg: String = "Scheduled time must be in the future.".orConfig

  val ScheduledChangeNotDueErrorMsg: String =
    "Cannot process scheduled change as it is not past the scheduled date of %s".orConfig

  val ManualReviewRequiresOwnerGroupErrorMsg: String =
    "Batch change requires owner group for manual review.".orConfig
  ...
  val UnsupportedKeyAlgorithmMsg: String =
    "Unsupported type for key algorithm, must be a string".orConfig
  ...
  val RecordSetTTLNotPositiveMsg: String =
    "RecordSet.ttl must be a positive signed 32 bit number".orConfig

  val RecordSetTTLNotValidMsg: String =
    "RecordSet.ttl must be a positive signed 32 bit number greater than or equal to 30".orConfig
  ...
  val NAPTRPreferenceValidationMsg: String =
    "NAPTR.preference must be an unsigned 16 bit number".orConfig

  val MissingNAPTRFlagsMsg: String = "Missing NAPTR.flags".orConfig

  val NAPTRFlagsValidationMsg: String = "NAPTR.flags must be less than 2 characters".orConfig
  ...
  val NAPTRRegexValidationMsg: String = "NAPTR.regexp must be less than 255 characters".orConfig

  val MissingNAPTRReplacementMsg: String = "Missing NAPTR.replacement".orConfig

  val NAPTRReplacementValidationMsg: String =
    "NAPTR.replacement must be less than 255 characters".orConfig

  val MissingSSHFPAlgorithmMsg: String = "Missing SSHFP.algorithm".orConfig

  val SSHFPAlgorithmValidationMsg: String =
    "SSHFP.algorithm must be an unsigned 8 bit number".orConfig
  ...
  val UnsupportedDigestTypeMsg: String =
    "Digest Type %d is not a supported DS record digest type".orConfig
  ...
  val CannotChangeZoneErrorMsg: String =
    "User '%s' cannot create or modify zone '%s' because they are not in the Zone Admin Group '%s'".orConfig
  ...
  val AccountAccessKeyErrorMsg: String =
    "Account with accessKey %s specified was not found".orConfig

  /* RecordSetChangeHandler.scala */

  val IncompatibleRecordMsg: String = "Incompatible record already exists in DNS.".orConfig

  val OutOfSyncMsg: String =
    "This record set is out of sync with the DNS backend; sync this zone before attempting to update this record set.".orConfig
  ...
  val CIDRErrorMsg: String = "PTR types must have no mask or a valid CIDR mask: %s".orConfig
  ...
  val InvalidDateFormatErrorMsg: String =
    "Invalid date format %s; provide the date format as YYYY-MM-DDTHH:MM:SSZ".orConfig
  ...
  val InvalidQueueNameErrorMsg: String =
    "Invalid queue name: %s. Must be 1-80 alphanumeric, hyphen or underscore characters. FIFO queues (queue names ending in \".fifo\") are not supported.".orConfig
