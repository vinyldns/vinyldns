...
class MySqlRecordSetRepositoryIntegrationSpec
  ...
  "list record sets" should {
    "return all record sets in a zone when optional params are not set" in {
      val existing = insert(okZone, 10).map(_.recordSet)
      val found = repo.listRecordSets(okZone.id, None, None, None, None, "asc").unsafeRunSync()
    ...
    "return record sets after the startFrom when set" in {
      ...
      val found = repo.listRecordSets(okZone.id, startFrom, None, None,None, "asc").unsafeRunSync()
    ...
    "return the record sets after the startFrom respecting maxItems" in {
      ...
      val found = repo.listRecordSets(okZone.id, startFrom, Some(2), None, None, "asc").unsafeRunSync()
    ...
    "return the record sets after startFrom respecting maxItems and filter" in {
      ...
      val found = repo.listRecordSets(okZone.id, startFrom, Some(3), Some("*z*"), None, "asc").unsafeRunSync()
    ...
    "return record sets using starts with wildcard" in {
      ...
      val found = repo.listRecordSets(okZone.id, None, Some(3), Some("aa*"), None, "asc").unsafeRunSync()
      (found.recordSets.map(_.name) should contain).theSameElementsInOrderAs(expectedNames)
    }
    "return record sets using ends with wildcard" in {
      ...
      val found = repo.listRecordSets(okZone.id, None, Some(3), Some("*b"), None, "asc").unsafeRunSync()
      (found.recordSets.map(_.name) should contain).theSameElementsInOrderAs(expectedNames)
    }
    "return record sets exact match with no wildcards" in {
      ...
      val found = repo.listRecordSets(okZone.id, None, Some(3), Some("aaa"), None, "asc").unsafeRunSync()
      (found.recordSets.map(_.name) should contain).theSameElementsInOrderAs(expectedNames)
    }
    "pages through the list properly" in {
      // load 5 records, pages of 2, last page should have 1 result and no next id
      val existing = insert(okZone, 5).map(_.recordSet).sortBy(_.name)
      val page1 = repo.listRecordSets(okZone.id, None, Some(2), None, None, "asc").unsafeRunSync()
      ...
      val page2 = repo.listRecordSets(okZone.id, page1.nextId, Some(2), None, None, "asc").unsafeRunSync()
      ...
      val page3 = repo.listRecordSets(okZone.id, page2.nextId, Some(2), None, None, "asc").unsafeRunSync()
