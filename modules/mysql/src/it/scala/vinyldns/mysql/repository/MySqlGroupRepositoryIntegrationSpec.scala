...
import vinyldns.core.domain.membership.{Group, GroupRepository, GroupStatus, MembershipStatus}
...
class MySqlGroupRepositoryIntegrationSpec
  ...
  private val groups = testGroupNames.map { testName =>
    Group(name = testName, email = "test@email.com", memberStatus = Some(MembershipStatus(Set(),Set(),Set())))
  }
