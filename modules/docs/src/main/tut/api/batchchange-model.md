...
status        | BatchChangeStatus | **PendingProcessing** - at least one change in batch has not finished processing; **Complete** - all changes have been processed successfully; **Failed** - all changes failed during processing; **PartialFailure** - some changes have failed and the rest are complete; **PendingReview** - one or more changes requires manual [approval](../api/approve-batchchange)/[rejection](../api/reject-batchchange) by a system administrator (ie. super or support user) to proceed ; **Rejected** - the batch change was rejected by a system administrator (ie. super or support user) and no changes were applied; **Scheduled** - the batch change is scheduled for a later date at which it needs to be approved to proceed. |
