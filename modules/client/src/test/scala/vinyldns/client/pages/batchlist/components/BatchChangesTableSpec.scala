...
class BatchChangesTableSpec extends WordSpec with Matchers with MockFactory with SharedTestData {
  ...
  val initialBatchChangeList = BatchChangeList(initialBatchChanges.toList, 100)

  class Fixture(changes: Option[BatchChangeList] = Some(initialBatchChangeList)) {
    val mockHttp = mock[Http]
    val props = BatchChangesTable.Props(mockHttp, mockRouter)

    (mockHttp.get[BatchChangeList] _)
