...
import vinyldns.client.pages.zoneview.components.RecordDataInput.MxInput.MxField.MxField
            ...
            S.recordSet.records.zipWithIndex.map {
              case (rd, index) =>
                <.tr(
                  ...
                  <.td(
                    <.input(
                      ^.className := s"form-control test-preference-$index",
                      ^.`type` := "number",
                      ^.value := Try(rd.preference.get.toString).getOrElse(""),
                  ...
                  <.td(
                    <.input(
                      ^.className := s"form-control test-exchange-$index",
                      ^.value := rd.exchange.getOrElse(""),
                      ^.onChange ==> { e: ReactEventFromInput =>
