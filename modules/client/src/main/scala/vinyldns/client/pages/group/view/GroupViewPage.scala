...
package vinyldns.client.pages.group.view
...
import vinyldns.client.pages.group.view.components.{MembersTable, NewMemberForm}
...
object GroupViewPage extends PropsFromAppRouter {
  case class State(group: Option[GroupResponse] = None)
  ...
  class Backend(bs: BackendScope[Props, State]) {
    ...
    def getGroup(P: Props): Callback = {
      ...
      val onSuccess = { (_: HttpResponse, parsed: Option[GroupResponse]) =>
    ...
    def getDescriptionHeader(group: GroupResponse): TagMod =
      group.description match {
        case Some(d) => <.h5(s"Description: $d")
        case None => TagMod.empty
      }

    def getIdHeader(group: GroupResponse): TagMod = <.h5(s"Id: ${group.id}")

    def getEmailHeader(group: GroupResponse): TagMod = <.h5(s"Email: ${group.email}")
  }
}
