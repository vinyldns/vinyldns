...
ALL_IDS=$(docker ps -a | grep -e 'vinyldns' -e 'flaviovs/mock-smtp'  -e 'rroemhild/test-openldap' | awk '{print $1}')
if [ "${ALL_IDS}" == "" ]; then
  echo "Nothing to remove"
  exit 0
fi

RUNNING_IDS=$(docker ps | grep -e 'vinyldns' -e 'flaviovs/mock-smtp' -e 'rroemhild/test-openldap' | awk '{print $1}')
if [ "${RUNNING_IDS}" != "" ]; then
